    ...
    >
      ...
      >
        <el-form-item :label="item.key">
          ...
          <areaChoose
            :firstLevel="searchQuery"
            :checkStrictly="item.checkStrictly"
            :expandTrigger="item.expandTrigger"
            :show="item.show"
            v-if="item.type === 'area'"
            :regionList="searchQuery[item.value]"
            v-model="searchQuery[item.value]"
            :level="item.level"
          ></areaChoose>
          ...
          <!-- 月日期选择器 -->
          <el-date-picker
            v-if="item.type === 'monthdate' || item.type === 'daydate'"
            v-model="searchQuery[item.value]"
            :class="`filter-${item.type}`"
            :type="item.type === 'monthdate' ? 'monthrange' : 'daterange'"
            range-separator="至"
            start-placeholder="开始时间"
            end-placeholder="开始时间"
            :format="item.type === 'monthdate' ? 'yyyy-MM' : 'yyyy-MM-dd'"
            :value-format="item.type === 'monthdate' ? 'yyyy-MM' : 'yyyy-MM-dd'"
          >
          </el-date-picker>
...
<script>
import areaChoose from '@/components/areaChoose/index'
export default {
  name: 'PageFilter',
  components: {
    areaChoose

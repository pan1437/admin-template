...
export default {
  ...
  /**
   * 导出Excel
   * @param {Array} tableInfo 表格数据
   * @param {Array} tHeader 表头
   * @param {Array} filterVal 表里的值
   * @param filename 表名
   */
  exportExcel(tableInfo, header, Val, filename) {
    import('@/vendor/Export2Excel').then(excel => {
      const tHeader = header //头
      const filterVal = Val //值
      const data = this.formatJson(filterVal, tableInfo)

...
(function () {
  ...
  var createPreviewIframeNode = function (editor, node) {
    ...
    if (name == 'video') {
      ...
      previewNode.attr({
        src: src,
        controls: 'controls', //鏄剧ず鎾斁宸ュ叿灏卞彇娑堣繖琛岀殑娉ㄩ噴
      ...
      let sourceSrc = vd.toLowerCase().split('type="')[1].split('"')[0];
      let source = new global$7('source', 1);
      source.attr({
        'src': src,
        type: sourceSrc
      });
      previewNode.append(source);
    } else {
      previewNode.attr({
        src: node.attr('src'),
        allowfullscreen: node.attr('allowfullscreen'),
    ...
    return function (nodes) {
      ...
      while (i--) {
        ...
        if (node.name === 'iframe' && Settings.hasLiveEmbeds(editor) && global$8.ceFalse) {
          ...
          if (!isWithinEmbedWrapper(node)) {
            //鑾嫢鍗块瓟鏀归儴鍒� node.replace(createPlaceholderNode(editor, node));
            node.replace(createPreviewIframeNode(editor, node));
          }
        }
      }
